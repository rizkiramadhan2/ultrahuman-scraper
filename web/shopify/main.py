...
import web.shopify.shopify as shopify
...
def main():
    try:
        ...
        with open(sys_path + "/web/shopify/url.json") as f:
        ...
        for web in web_list_to_scrape:
            ...
            for product in products:
                ...
                title = f"{product.get("option2", "")} {size}"
                price = product.get("price", "")
                stocks = "in stock" if product.get("available", "") else "out of stock"
                rating = product.get("rating", "")

                if not data_map.get(id):
                ...
                if data_map[id]["price"] != price:
                    if id in diff_data:
                        diff_data[id].append(
                            {
                                "title": title,
                                "price": data_map[id]["price"],
                                "new_price": price,
                            }
                        )
                    else:
